...
Page({
  ...
  async loadCraftDetail(craftId) {
    ...
    const phases = result.phases.map((p, index) => ({
      ...p,
      // 第一阶段默认解锁，后续需前一阶段完成
      unlocked: index === 0 || result.userProgress?.unlockedPhases?.includes(p.id),
      completed: result.userProgress?.phaseProgress?.[p.id] || 0
    }));
    
    const completedSteps = result.userProgress?.completedSteps?.length || 0;
