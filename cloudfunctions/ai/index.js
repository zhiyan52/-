...
exports.main = async (event, context) => {
  const { action, question, systemPrompt } = event

  try {
    if (action === 'chat') {
      // 调用腾讯云DeepSeek生文模型
      const answer = await callDeepSeekModel(question, systemPrompt)
...
// 调用DeepSeek生文模型
async function callDeepSeekModel(question, systemPrompt) {
  try {
    // 调用腾讯云AI能力
    // 这里使用云开发的AI能力
    ...
    // 如果调用失败，使用本地智能回答作为备用
    return generateIntelligentAnswer(question, systemPrompt)
  } catch (error) {
    console.log('DeepSeek模型调用失败，使用本地智能回答:', error)
    // 调用失败时使用本地智能回答
    return generateIntelligentAnswer(question, systemPrompt)
  }
}
...
// 生成智能回答（备用）
function generateIntelligentAnswer(question, systemPrompt) {
